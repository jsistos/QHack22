...
def error_wire(circuit_output):
    ...
    density_matrix = np.array(circuit_output, dtype=float)

    probs = []
    probs.append(density_matrix[0,0] + density_matrix[7,7])
    for i in range(3):
        probs.append(density_matrix[3-i,3-i] + density_matrix[4+i,4+i])
